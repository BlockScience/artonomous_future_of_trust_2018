...
contract KittyBase is KittyAccessControl {
    ...
    event Vote(uint256 candidate, uint256 voteCount, uint256 currentGenerator, uint256 currentGeneratorVoteCount);
...
contract KittyMinting is KittyAuction {
    ...
    uint256 public constant PROMO_CREATION_LIMIT = 250;
    ...
    function createPromoKitty() external onlyCOO {
        address kittyOwner = recipients[promoCreatedCount];
        ...
        promoCreatedCount++;
        // uint16 _generator = uint16(uint64(now) % 2);
        // if (votes[0] > votes[1]) {
        //     _generator = 0;
        // } else if (votes[1] > votes[0]) {
        //     _generator = 1;
        // }
        _createKitty(0, 0, curGenerator, 0, kittyOwner);
    }

    uint[] votes;
    uint public curGenerator = 0;

    function castVote(uint _generator) external {
        votes[_generator] = votes[_generator] + 1;
        if (votes[_generator] > votes[curGenerator]) {
            curGenerator = _generator;
        }
        Vote(_generator, votes[_generator], curGenerator, votes[curGenerator]);
    }

    address[] recipients;

    function addRecipient(address _recipient) external {
        uint _id = recipients.push(_recipient);
        NewRecipient(_recipient, _id);
    }
